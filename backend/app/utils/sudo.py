...
async def run_sudo(cmd: List[str], timeout: int = 30, env: dict = None) -> Dict[str, object]:
    ...
    Allocates a PTY and runs the subprocess in a new session, making the PTY
    the controlling terminal via TIOCSCTTY. This satisfies sudo's
    'Defaults requiretty' on RHEL/enterprise systems even when the GUI
    runs as a systemd service (no real TTY).
    ...
    try:
        def preexec() -> None:
            # New session + make this pty the controlling terminal for the child.
            os.setsid()
            try:
