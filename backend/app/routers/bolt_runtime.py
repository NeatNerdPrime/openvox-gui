...
def write_estate_bolt_inventory(
    ...
    Does **not** use PuppetDB or openvox_enc plugins (orchestration can still
    use ENC plugin inventory separately). Avoids reading root-only
    ...
    if not uris:
        # Minimal stub so Bolt still starts; caller may pass URI targets.
    ...
    # Optional ENC group for orchestration (local GUI) — separate from estate list
    lines.extend(
        [
            "groups:",
            "  - name: enc",
...
def sanitize_bolt_inventory(
    ...
    """Hand Bolt a GUI-writable inventory (estate FQDNs + SSH). Never require /etc."""
    del src  # never trust on-disk inventory from old GUI sync / root-only files
    try:
        return write_estate_bolt_inventory(
            dest="/opt/openvox-gui/data/bolt-inventory.estate.yaml"
        )
    except OSError:
        # Last resort: write minimal file next to dest
        out = Path(dest)
        try:
            out.parent.mkdir(parents=True, exist_ok=True)
            out.write_text(
