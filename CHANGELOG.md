...
### Fixed
- **install.bash Ubuntu/.deb path:** after ``match:`` the script
  curled the package index with no timeout and no log line, so a
  hung or 404 index looked like a freeze. Curls now have connect
  and max times and fail loudly on an empty index.
...
### Fixed
- **Node Detail health glance:** ``/var/run``, ``/run``, and other
  tmpfs/proc-style mounts no longer appear as 100% full disks.
...
### Changed
- Replaced estate FQDNs in docs, Settings placeholders, script
  examples, and comments with `example.com`. Leak scan now rejects
  the former internal DNS suffix.
...
### Fixed
- **Certificate Sign** from either dedicated console now discovers
  the Promoted CA **at click time**.
...
### Fixed
- Login failure messages no longer point at a non-existent
  "Settings → Application → Auth" path. Correct GUI paths are
  **Settings → Auth Settings** (LDAP) and **Settings → User Manager**
  (per-user auth source). Login error text wraps fully in the alert.
...
### Changed
- **Node Detail:** restore Overview | Applied Classes on the top row; **Host health**
  glance spans full width below that and above Key Facts / All Facts / Reports.
...
### Fixed
- **Infrastructure | Orchestration Overview:** Version showed `^@5.6.0`
  because `bolt --version` stdout started with a NUL. Strip control
  characters and display the SemVer token only (`5.6.0`).
...
### Fixed
- **Classification | Environments:** after sync, the table could stay empty
  because the list was filtered incorrectly and the toast fired even when
  creates failed. Always render one row per discovered control_repo
  environment, merge ENC classes/parameters, and only report success for
  real loads.
...
### Changed
- **Classification | Environments:** environment **names** are auto-populated from
  control_repo (via compilers/r10k discovery). Operators can only edit
  **classes and parameters** — no Add Environment / Delete / rename.
  Resync button re-pulls branch list from compilers.
...
### Fixed
- **Classification | Environments** banner no longer claims discovery from
  `/etc/puppetlabs/code/environments/` on the GUI host. Copy matches
  compiler/PuppetDB discovery (dedicated console has no control_repo).
...
### Fixed
- **Insights | Monitoring / Run Performance:** `scope is not defined` crash —
  `MetricsPerformanceContent` used `scope` without receiving it as a prop from
  the parent page.

## [3.11.1-alpha.9] - 2026-08-13 (fix — Postgres user create / bcrypt)

### Fixed
- **manage_users / local auth on PostgreSQL:** User `created_at`/`updated_at` use
  timezone-naive UTC so asyncpg accepts `TIMESTAMP WITHOUT TIME ZONE`.
- **bcrypt pin 4.0.1** (passlib 1.7.4 incompatible with bcrypt 4.1+ `__about__` removal).
...
### Removed
- `docs/COMPILER_ENC.md`, `docs/OPENVOX_GUI_SHARED_DB.md`,
  `etc/openvox-enc.sysconfig.example` and installer/doc cross-links to them
  (session runbooks; product direction is single console + remote app DB).
...
### Fixed
- **OpenBolt project name** in `install.sh` and `enable-console-orchestration.sh` is `openvox_gui` (underscore). Bolt rejects hyphens: `openvox-gui` fails with “project name must begin with a lowercase letter…”.
...
### Added
- **`scripts/enable-console-orchestration.sh`**: idempotent root script for dedicated consoles. Creates `/etc/puppetlabs/bolt`, `bolt` user, `openvox_enc`, `id_bolt`, copies agent PEMs for the bolt user, writes OpenVoxDB `puppetdb-all` inventory from GUI `.env`.
...
## [3.11.0-alpha.4] - 2026-08-06 (docs — migration stance)

### Changed
- **Estate strategy:** migrate nodes to the new multi-server cluster; convert the former production singleton into a **development instance** (documented in INSTALL Advanced Installations + AGENTS). No in-place production cutover assumed.
...
- **INSTALL.md — Advanced Installations (Extra Large / Clustered Estates)** at the bottom of the guide:
  ...
  - **Migration stance:** new cluster + migrate nodes; former production becomes a **dev instance** (not in-place production cutover)
...
### Fixed
- **`deploy.sh` / remote update** now rewrites `openvox-gui.service` from the repo template (workers, concurrency limits, `LimitNOFILE`, SSL flags) instead of leaving a stale lab dual-uvicorn launcher in place. Remote deploys finally pick up serving resource settings.
...
### Fixed
- **Executive Summary Send → API 500**: in-process snapshot `json.dump` failed on non-JSON values from PuppetDB/services (e.g. `datetime`). Serialize with a safe default encoder and return a clear error body if dump still fails.
...
### Fixed (carried from 3.10.2-N train into this beta)
...
### Versioning
- **Bugfix on the 3.10.2 line** (not a new minor). Display/install version is **`3.10.2+bugfix`** (PEP 440 **local** segment — required so `ovox` / pip metadata install cleanly; a literal `3.10.2-bugfix` is **not** valid PEP 440 and broke remote deploy).
- Subsequent fixes on this line: **`3.10.2+bugfix1`**, **`+bugfix2`**, … Git tag / friendly GH name may still use **`v3.10.2-bugfix`** for the first cut; numbered tags prefer **`v3.10.2+bugfixN`**.
- Supersedes the brief **`3.10.3`** label on the same change set (tag/release removed where possible).
...
### Documentation
- Added screenshots for the new metrics views to the 📸 Screenshots section in README.md.
  - Performance Metrics (`performance.png`)
  - OpenVox Server Health (`server.png`)
  - OpenVox DB Health (`db.png`)
- Formatted the new screenshots and aligned them in the table with the existing Dashboard, Node Details, and Orchestration entries.
...
- Made OpenVox Server Health graphs actually receive and display data:
  - More robust recursive search across the entire /status response to locate the experimental.http-metrics list (handles "http-metrics" or "http-client-metrics" keys, variations in structure between dev and prod).
  - Parser now handles both "route-id" and "metric-id" (array or string) for identifying catalog/total routes, plus super-fallback on str(item) for prod differences.
...
- The "experimental http metrics" (route means for Catalog/Total) are the http-client metrics. Enable with `metrics-enabled = true` in the `http-client` section of puppetserver.conf (defaults to true). This is separate from metrics.conf (which controls registries/JMX).
...
### Bug Fixes
- **Dashboard | Overview "Nodes" pane**: Was not scrolling internally (using mah=600, showing limited nodes without functional scrollbar). Matched the successful direct explicit height pattern from Dashboard | Nodes "All Nodes" (and submenus): changed to `<ScrollArea h={650} ...>` directly, plus `style={{ overflow: 'hidden' }}` on the Card. This creates a consistent scrollable region (~15 rows visible) for the full list of nodes while the page remains scrollable.
- Followed hyphenated versioning policy on 3.7.33 base.
