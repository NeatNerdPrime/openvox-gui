...
for script in enc.py manage_users.py deploy.sh r10k-deploy.sh r10k-stage-activate.sh update_local.sh sync-openvox-repo.sh ensure-sudoers.sh generate_fleet_health_report.py ca-reject-csr.sh enable-console-orchestration.sh fix-console-bolt-inventory.sh apply-singleton-bolt-layout.sh; do
...
# Check common control-repo locations first. If the fact is already present
# (and executable) in e.g. site/profiles/facts.d/, it should be automatically
# pluginsynced to classified nodes — no need to nag.
...
if [ -n "$DETECTED_PUPPET_AGENT_DISABLED_FACT" ]; then
    ...
    log_info "  Pluginsync (or equivalent) assumed for classified nodes."
fi

# Deploy/update the puppet_agent_disabled external fact (for Node Health)
# This is a convenience copy on the GUI server only (for reference / copy-paste).
# The actual fact is expected to live in your control repo so it gets
# automatically pluginsynced.
...
if [ -f "$FACT_SRC" ]; then
    ...
    if [ "$WAS_PRESENT" = "false" ]; then
        ...
        if [ -z "$DETECTED_PUPPET_AGENT_DISABLED_FACT" ]; then
...
#
# See scripts/ensure-sudoers.sh for the full policy explanation:
#   - Only our file is ever touched.
#   - Automatic .bak backup is made before any overwrite.
#   - No rm of any other sudoers.d entries.
...
if [ -n "$DETECTED_PUPPET_AGENT_DISABLED_FACT" ]; then
    echo -e "    Detected in control repo at ${DETECTED_PUPPET_AGENT_DISABLED_FACT}"
    echo -e "    (pluginsync assumed — no further action needed for classified nodes)."
else
