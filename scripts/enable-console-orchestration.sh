...
# OpenVoxDB connection MUST be in bolt-project.yaml, not inventory config.
...
echo "Wrote $BOLT_DIR/bolt-project.yaml (OpenVoxDB https://${PDB_HOST}:${PDB_PORT})"

if [[ ! -f $BOLT_DIR/inventory.yaml ]] || grep -q '^  puppetdb:' "$BOLT_DIR/inventory.yaml" 2>/dev/null; then
...
# Connection settings live in bolt-project.yaml.
version: 2
config:
  transport: ssh
  ssh:
...
EOF
  ...
  echo "Wrote $BOLT_DIR/inventory.yaml"
else
  echo "Left existing $BOLT_DIR/inventory.yaml in place (no puppetdb: under config)"
fi
